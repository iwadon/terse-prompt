...
static void buffer_row_to_string(const tprompt_screen_buffer_t *buffer,
	size_t row, char *out, size_t out_size)
{
	if (!buffer || !out || out_size == 0 || row >= buffer->rows) {
		if (out && out_size > 0) {
			out[0] = '\0';
		}
		return;
	...
	size_t out_pos = 0;
	for (size_t col = 0; col < buffer->cols && out_pos + 1 < out_size; col++) {
		const tprompt_screen_cell_t *cell = &buffer->cells[row * buffer->cols + col];
		if (cell->char_len == 0) {
			out[out_pos++] = ' ';
			continue;
		}
...
TEST(StatusLine, CustomCallbackRendersLine)
{
	...
	char row_text[128];
	buffer_row_to_string(&handle->display.previous_buffer, 1, row_text, sizeof(row_text));
